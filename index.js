...
io_101.on('connection', function (socket) {
    ...
    socket.on('a message from the client to the server', function (parameter) {
        console.log("The server received the following message from " + socket.id + ": " + parameter);
    })

    socket.emit('a message from the server to the client', 'The socket says, "Hello"');
    socket.broadcast.emit('a message from the server to the client', 'The socket says, "Hello" to all other sockets');
    io_101.emit('a message from the server to the client', 'The server says, "Hello" to all sockets')
});
